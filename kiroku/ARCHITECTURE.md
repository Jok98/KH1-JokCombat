...
6. Exit, reload, timeout, menu, state, and fault paths conditionally restore temporary values from signed journals or snapshots.

## Boundaries

- `JokCombat_CombatPrototype.lua`: combat state machine, A/Y families, Action and Limit routing, defense, movement, Command Menu guide, R2 magic page, journals, and runtime recovery.
...
- `JokCombat_StateProbe.lua`, `JokCombat_InputProbe.lua`, and `JokCombat_CommandMenuProbe.lua`: optional diagnostics, never release dependencies.
...
- Static verification: five `tests/test_*.py` scripts.
- Lua harnesses: three `tests/*_test.lua` files; syntax compilation should cover all seven root Lua scripts.
